...
def _simple_pdf(lines):
    ...
    objects.append(b"3 0 obj << /Type /Page /Parent 2 0 R /MediaBox [0 0 612 792] /Contents 5 0 R /Resources << /Font << /F1 4 0 R >> >> >> endobj")
...
def export_single_student_pdf(student_id=None):
    ...
    lines = [
        ...
        f"Stability Score: {student.get('stability_score', 0)}%",
        f"Category: {student.get('category', '')}",
        f"Trend Change: {student.get('trend_diff', 0)}%",
        f"Test Score: {student.get('test_score', 0)}%",
        f"Attendance: {student.get('attendance', 0)}%",
        f"Assignment: {student.get('assignment', 0)}%",
        "-" * 80,
        "Subject Breakdown:",
        f"Tamil: {student.get('tamil', 0)}%",
        f"English: {student.get('english', 0)}%",
        f"Maths: {student.get('maths', 0)}%",
        f"Science: {student.get('science', 0)}%",
        f"Social Science: {student.get('social_science', 0)}%"
    ]
    pdf_bytes = _simple_pdf(lines)
